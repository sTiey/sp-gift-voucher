...
/* ⚠️ ประตูนี้ต้อง "นุ่ม" ห้ามเป็นเปิด-ปิดเด็ดขาด — ไท้เจอเองรอบสอง 2026-08-19
      ของเดิมสั่งว่า "เร็วเกิน 9 องศา/วินาที = กำลังเอียงอยู่ ห้ามแตะศูนย์"
      แต่ **มือคนถือมือถือแกว่งเกิน 9 องศา/วินาทีเป็นเรื่องปกติ** ประตูจึงปิดเกือบตลอด
      พอบวกกับมือที่ไหลช้า ๆ ตามธรรมชาติ ศูนย์ก็ไล่ไม่ทัน = ค้างค้างอยู่อย่างนั้น
      (ไท้วัดได้ค้างที่ 14 องศา หลังถือนิ่ง 12 วินาที · ทำซ้ำได้ด้วยแบบจำลองมือคน)
   เปลี่ยนเป็นตัวคูณที่ลดลงตามความเร็ว **แต่ไม่มีวันเป็นศูนย์** → ไม่มีวันค้างถาวรอีก
      ยิ่งเอียงเร็ว ยิ่งเข้าใกล้ 0 (เอฟเฟกต์เต็มระยะ) · อยู่นิ่ง = 1 (ไล่ศูนย์เต็มที่) */
const GYRO_CALM = 12;   /* องศา/วินาที — จุดที่ตัวคูณเหลือครึ่งหนึ่ง */
/* ⚠️ ต้องกรองมือสั่นออกก่อนวัดความเร็ว ไม่งั้นตัวไล่ศูนย์จะไม่มีวันทำงานบนมือถือจริง
   ไท้เจอเอง 2026-08-19: วางบนโต๊ะแล้วคลายกลับได้ แต่ถือในมือค้าง 12 วินาทีไม่ไปไหน
   ต้นเหตุ: เดิมวัดความเร็วจาก "สัญญาณสองตัวที่ติดกัน" ซึ่งบน iPhone ห่างกัน 0.017 วินาที
     มือสั่นแค่ 0.15° ก็คิดเป็น 9°/วินาที = ถูกอ่านว่ากำลังเอียงตลอดเวลา → ไล่ศูนย์ไม่เคยทำงาน
   วัดจากค่าที่ผ่านการเกลี่ยแล้วแทน — มือสั่นถูกหักออก แต่การเอียงจริงยังวัดได้เต็ม
   วัดจริงบน Chrome (มือสั่น 0.4°): ของเดิมค้างไม่กลับเลย · แบบนี้กลับเข้ากลางใน 1.1 วินาที
   และระยะที่ประกายกวาดยังเท่าเดิม (30-34%) ไม่ได้แลกความแรงของเอฟเฟกต์ไป */
const SPD_TAU = 0.18;   /* วินาที — เวลาเกลี่ยก่อนเอาไปวัดความเร็ว */
/* ⚠️ ค่านี้คุมอาการ "ค้างไม่เข้ากลาง" ตอนถือในมือจริงมากที่สุด ไม่ใช่ค่า EDGE
      ไท้เทสรอบ 2026-08-21 เหลือค้างที่ 12 องศาหลังถือ 12 วินาที — ยังไม่เข้ากลางสนิท
      ลองปรับ "ความทน" (GYRO_CALM) แล้ว **ซื้อไม่คุ้ม**: อาการดีขึ้นนิดเดียว
      แต่ประกายกวาดหายไป 15% (127 เหลือ 112) — จึงคืนค่าเดิม
      ปรับค่านี้แทนคุ้มกว่ามาก: ค้างจาก 14 เหลือ 8 · กวาดปกติ 127 เหลือ 125 (แทบไม่ขยับ) */
const GYRO_FOLLOW_SLOW = 0.50;   /* ต่อวินาที — เอียงนิดเดียวแล้วค้างไว้ อยู่ได้ราว 2 วินาที */
const GYRO_FOLLOW_EDGE = 3.00;   /* ตอนค้างสุดขอบ — ไล่กลับในราว 0.3 วินาที */
...
function onOrientation(ev) {
  ...
  /* ⚠️ ไล่ศูนย์ **เฉพาะตอนเครื่องอยู่นิ่ง** เท่านั้น
     ...
     กำลังเอียง = ไล่ช้าลงมาก · หยุดนิ่งเมื่อไหร่ค่อยไล่เต็มที่
     ⚠️ ของที่แลกไป: เอียงช้ามาก ๆ (3-4 วินาทีต่อข้าง) ระบบจะเริ่มนับว่าเป็นท่าถือใหม่
        ประกายกวาดเหลือ 81% จาก 115% — ยอมแลก เพราะอาการค้างเจอทุกครั้ง แต่เอียงช้าขนาดนั้นแทบไม่มีใครทำ */
  const sa = 1 - Math.exp(-dt / SPD_TAU);
  if (gyro.smX === null) { gyro.smX = sx; gyro.smY = sy; }
  const pX = gyro.smX;
  const pY = gyro.smY;
  gyro.smX += (sx - gyro.smX) * sa;
  gyro.smY += (sy - gyro.smY) * sa;
  const spd = dt > 0 && gyro.lastAt ? Math.hypot(gyro.smX - pX, gyro.smY - pY) / dt : 0;
  gyro.lastAt = at;
  const calm = 1 / (1 + (spd / GYRO_CALM) * (spd / GYRO_CALM));
  const k = 1 - Math.exp(-(GYRO_FOLLOW_SLOW + (GYRO_FOLLOW_EDGE - GYRO_FOLLOW_SLOW) * r * r) * calm * dt);
  gyro.zero.x += dx * k;
  gyro.zero.y += dy * k;
...
function startGyro() {
  ...
  /* ⚠️ เคยสั่งให้ "จำท่าถือใหม่" ตอนหมุนจอ — ผิด และเป็นต้นเหตุที่ไท้เจอ 2026-08-19
     ("พอกลับมาเป็นแนวตั้งจากแนวนอน บัตรก็จะเอียง ๆ ทั้งที่มือถือไม่ได้เอียง")
     เพราะมันไปจำใหม่ตอนที่มือกำลังหมุนเครื่องอยู่พอดี ได้ค่าอ้างอิงกลางอากาศ
     ที่จริง **ไม่ต้องจำใหม่เลย** — ค่าที่เก็บอยู่ในแกน "ตามที่ตาเห็นบนจอ"
     ซึ่งหมุนเครื่องในระนาบเดิมยังไงก็ไม่เปลี่ยน (พิสูจน์ด้วยสูตรและวัดจริงแล้ว)
     วัดจริง (ควรได้ 0%): ของเดิมเพี้ยน 42.5% ตอนแนวนอน · 39.2% ตอนกลับเป็นแนวตั้ง
                          แบบนี้เหลือ 0.5% และ 0.8% */
}
